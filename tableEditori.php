    ...
    */
    ...
    class TableEditor
    {
        ...
        */
        function TableEditor($db, $table)
        {
            ...
            */
            ...
            $this->config['closeOnCancel']  = false;
        ...
        */
        ...
        /**
        * Sets default values for copies. Particularly useful for when fields
        * are hidden from users on the add page
        *
        * @param array $arr Associative array of default values, keyed by field name,
        *                   values are the default values.
        */
        function setCopyDefaultValues($arr)
        {
            if (is_array($arr)) {
                foreach ($arr as $field => $v) {
                    if (!empty($this->fields[$field])) {
                        $this->fields[$field]['copydefault'] = $v;
                    }
                }
            }
        }
        ...
        *                   values are the default values.
        */
        function setEditDefaultValues($arr)
        {
            if (is_array($arr)) {
                foreach ($arr as $field => $v) {
                    if (!empty($this->fields[$field])) {
                        $this->fields[$field]['editdefault'] = $v;
                    }
                }
            }
        }
        ...
        */
        function addDisplayFilter($field, $callback)
        {
            if (is_callable($callback) AND isset($this->fields[$field])) {
                ...
                $this->errors[] = "Failed to apply Display Filter - not a valid PHP Filter";
            }
        }

        /**Function added by CRANE for seperate view page filters
        */
        
        function addViewFilter($field, $callback)
        {
            if (is_callable($callback) AND isset($this->fields[$field])) {
                $this->fields[$field]['viewfilters'][] = $callback;
        ...
        */
        ...
        /**  ADDED BY CRANE TO APPLY VIEW FILTERS
        * Applys view filters to a single row of data. Does htmlspecialchars() first.
        *
        * @param array &$results Data from table
        */
        function applyViewFilters(&$row)
        {
            foreach ($row as $field => $value) {
                if (!empty($this->fields[$field]['viewfilters'])) {
                    foreach ($this->fields[$field]['viewfilters'] as $f) {
                        $value = call_user_func($f, $value);
                    }
        ...
        */
        function display()
        {
            ...
            */
            if (isset($_GET['edit'])) {
                if ($this->getConfig('allowEdit')) {
                    if($_GET['webid']== 1) $this->pk = 'web_id';
            ...
            */
            foreach ($this->fields as $field => $v) {
                if (empty($v['noDisplay'])) {
                    ...
                    rsort ($this->fields[$field]['filters']);   ################ADDED BY CRANE  Reverses the Display filter putting 'htmlspecial' first#######
                }
            }
            ...
            */
            $this->displayHeader();
?>
<table border="0" align="left" id="dataTable">
    <tr>
        <td>&nbsp;</td>
        <td colspan="<?=count($this->fields)?>" align="left">
...
<?php
        ...
        */
        function displayActionButtons()
        {
            ...
            echo '<button onclick="select_all(\'dataTable\')" class="actionButton">Select All</button>';
        ...
        */
        function handleView($id)
        {
            ...
            */
            ...
            $this->applyViewFilters($row);  ####added to apply display filter for view mode by CRANE
        ...
        function handleAddEditCopy($id = null)
        {
            ...
            if (!empty($_POST)) {
                ...
                if ($_POST['action'] == 'Cancel') {
                    if($this->getConfig('closeOnCancel') == true){
            ...
            if (isset($_GET['edit']) OR isset($_GET['copy'])) {
                ...
                */
                ...
                if ($row === false) {
  			    ...
  			    foreach ($row as $k => $v) {
                     $row[$k] = isset($this->fields[$k]['editdefault']) ? $this->fields[$k]['editdefault'] : $v;
                 }

                // If copying, nuke the primary key so we don't get conflicts on insert
                // Set the value to the copy default value
                if (isset($_GET['copy'])) {
                
                    $row[$this->pk] = '0';
                    foreach ($row as $k => $v) {
                         $row[$k] = isset($this->fields[$k]['copydefault']) ? $this->fields[$k]['copydefault'] : $v;
                     }
...
<table border="0" >
            ...
            <tr>
                ...
                <td>
                    <?php
                        switch ($this->fields[$field]['input']) {
                            case 'textarea':
                                printf('<textarea name="%s" cols="60" rows="5" onkeyup="enableApply()" %s>%s</textarea>',
                            ...
                            case 'date':
                                printf('<input type="text" name="%s" value="%s" onkeyup="enableApply()" %s> <a href="javascript: void(document.forms[0].elements[\'%s\'].value = currentDate())" onclick="enableApply()" title="Click to set current date">Today</a>',
                                       htmlspecialchars($field),
                                       htmlspecialchars($value),
                                       $field == $this->pk && !$this->getConfig('allowPKEditing') ? 'disabled': '',
                                       htmlspecialchars($field));
                            ...
                            default:
                                printf('<input type="text" size="70" name="%s" value="%s" onkeyup="enableApply()" %s>',
                                       htmlspecialchars($field),
                                       htmlspecialchars($value),
                                       $field == $this->pk && !$this->getConfig('allowPKEditing') ? 'disabled': '');
                                break;
    ...
    <tr>
        <td colspan="3" align="right">
            ...
            <? if($this->getConfig('showCancel')) echo'<input type="submit" name="action" style="width: 65px" value="Cancel">'; ?>
   ...
   <!-- <link rel="stylesheet" href="./style.css">!-->
    ...
    <!--
        ...
        */
        ...
        function select_all(id){
    		...
    		for(i = 3; i < rows.length-2; i++){
      			rows[i].className = "highlightedRow";
        ...
        function row_highlight(event, trObj, checkboxObj, origClass)
        {
            ...
            if (srcElement.tagName.toLowerCase() == 'td' || srcElement.tagName.toLowerCase() == 'tr' || srcElement.tagName.toLowerCase() == 'img') {
        ...
        */
        ...
        {
            document.forms[0].elements['action'][1].disabled = false;
...
<?php
        ...
        */
        function dbGetRow($sql)
        {
            ...
            if ($res = $this->dbQuery($sql) AND mysqli_num_rows($res) > 0) {
